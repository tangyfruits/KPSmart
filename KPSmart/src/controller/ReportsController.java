...
public class ReportsController extends NavBarController {
	...
	private void routeLoadAction() {
		...
		for (Tuple t : temp.keySet()) {
			RouteLoadRow row = new RouteLoadRow(t.getOrigin(), t.getDestination(),
					odf.format(temp.get(t).get(0)) + " g", odf.format(temp.get(t).get(1)) + " cm\u00b3",
					sf.format(temp.get(t).get(2)));
			report.add(row);
		}
	...
	private void criticalRoutesAction() {
		...
		for (TuplePriority t : temp.keySet()) {
			CriticalRoutesRow row = new CriticalRoutesRow(t.getOrigin(), t.getDestination(), t.getPriority(),
					"$" + df.format(temp.get(t).get(0)), "$" + df.format(temp.get(t).get(1)),
	...
	private void averageTimesAction() {
		...
		for (TuplePriority t : temp.keySet()) {
			...
			AverageTimesRow row = new AverageTimesRow(t.getOrigin(), t.getDestination(), t.getPriority(),
					odf.format(average) + " hrs");
			avReport.add(row);
		}
