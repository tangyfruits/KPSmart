...
public class LogDiscontinueRouteTests {
	...
	public void test4() {
		...
		assertEquals(3, main.getLocations().get(0).getRoutes().size());
		// discontinue route
		main.discontinueTransportRoute("Wellington", "Auckland", "PostHaste", "Air", false, LocalDateTime.now(),
				"User");
	...
	public Main setUpRoutes() {
		...
		main.logTransportCostUpdate("Wellington", "Auckland", "NZPost", "Air", 2.5, 2.8, 8, 12, 12, 32,
				DayOfWeek.MONDAY, 15, false, LocalDateTime.now(), "User");
		main.logTransportCostUpdate("Wellington", "Auckland", "USPS", "Air", 2.5, 2.8, 8, 12, 12, 32, DayOfWeek.MONDAY,
				15, false, LocalDateTime.now(), "User");
		return main;
